...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2)
	{
		...
		List<WPTreeNode> wordsToExplore = new LinkedList<WPTreeNode>();
		HashSet<String> visited = new HashSet<String>();

		this.root = new WPTreeNode(word1, null);
		visited.add(word1);
		wordsToExplore.add(this.root);
		WPTreeNode currentNode = this.root;

		while(wordsToExplore.size() != 0 && !currentNode.getWord().equals(word2))
		{
			currentNode = wordsToExplore.remove(0);
			List<String> neighbours = nw.distanceOne(currentNode.getWord(), true);
			for(String currentWord : neighbours)
			{
				if(!visited.contains(currentWord))
				{
					WPTreeNode childNode = currentNode.addChild(currentWord);
					visited.add(currentWord);
					wordsToExplore.add(wordsToExplore.size(), childNode);
